...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, label_map):
        ...
        Returns:
            ...
            labelmap : [num_input_classes, 256]
        """

        bs, num_queries = outputs["pred_logits"].shape[:2]

        # We flatten to compute the cost matrices in a batch
        out_prob = outputs["pred_logits"].flatten(0, 1).sigmoid()  # [1 * num_queries, num_classes]
        out_bbox = outputs["pred_boxes"].flatten(0, 1)  # [1 * num_queries, 4]

        # Also concat the target labels and boxes
        tgt_ids = torch.cat([v["labels"] for v in targets])  # true label id, allows repeat
        tgt_bbox = torch.cat([v["boxes"] for v in targets])  # true bbox
        ...
        new_label_map = label_map[
            tgt_ids.cpu()
        ]  # 返回true lable id对应的labelmap，有重复的id返回重复的行，shape=[num_true_id, 256]

        neg_cost_class = (1 - alpha) * (out_prob**gamma) * (-(1 - out_prob + 1e-8).log())  # 900,256
        pos_cost_class = alpha * ((1 - out_prob) ** gamma) * (-(out_prob + 1e-8).log())  # 900,256
        new_label_map = new_label_map.to(pos_cost_class.device)  # [len]

        cost_bbox = torch.cdist(out_bbox, tgt_bbox, p=1)  # 输出box和真实box集合之间的L1距离，[900, len(true_box)]

        # cost_class=(pos_cost_class @ new_label_map.T - neg_cost_class@ new_label_map.T)
        cost_class = []
        for idx_map in new_label_map:  # [256]
            idx_map = idx_map / idx_map.sum()  # 归一化
